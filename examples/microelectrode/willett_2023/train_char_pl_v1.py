        ...
        for iterIdx in range(pred.shape[0]):
            ...
            trueSeq_decodedSeq_text.append(
                [
                    self.current_epoch,
                    iterIdx,
                    matcher.distance() / len(trueSeq),
                    "".join(map(idToChar, trueSeq)),
                    "".join(map(idToChar, decodedSeq)),
                ]
            )

        self.logger.log_text(
            key="samples",
            columns=["EPOCH", "IDX", "PER", "trueSeq", "decodedSeq"],
...
@hydra.main(version_base=None, config_path="configs", config_name="config_char")
def main(cfg: DictConfig) -> None:
    ...
    cfg.base.exp_name = f"{cfg.base.date}_{cfg.base.model_name}"
