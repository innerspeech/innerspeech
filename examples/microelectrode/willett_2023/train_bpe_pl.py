...
class Lightning_GRUDecoder(L.LightningModule):
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.current_epoch % 10 == 0:
            ...
            for iterIdx in range(pred.shape[0]):
                ...
                trueSeq_decodedSeq_text.append(
                    [
                        ...
                        jiwer.wer(trueSeq_text, decodedSeq_arpa_text),
                        trueSeq_text,
                        decodedSeq_text,
                        decodedSeq_arpa_text,
                    ]
                )
            # get the list of all trueSeq_text
            all_trueSeq_text = [x[-3] for x in trueSeq_decodedSeq_text]
            all_decodedSeq_text = [x[-2] for x in trueSeq_decodedSeq_text]
            ...
            dev_wer_lm = jiwer.wer(all_trueSeq_text, all_decodedSeq_arpa_text)

            self.logger.log_text(
                key="samples",
                columns=["EPOCH", "IDX", "WER", "WER_lm", "trueSeq", "decodedSeq", "decodedSeq_lm"],
